...
- Anleitung auf deutscch schreiben
...
| Funktion   | Beschreibung                                                                 | Beispiel                                      |
|------------|------------------------------------------------------------------------------|-----------------------------------------------|
| `len()`    | Gibt die Anzahl der Elemente eines Objekts zurück.                           | `len([1, 2, 3])` gibt `3` zurück              |
| `type()`   | Gibt den Typ eines Objekts zurück.                                           | `type(42)` gibt `<class 'int'>` zurück        |
...
| `sort()`   | Sortiert eine Liste direkt (in-place), d. h., die ursprüngliche Liste wird verändert. | `liste = [3, 1, 2]`<br>`liste.sort()`<br>`print(liste)` gibt `[1, 2, 3]` zurück |
| `sorted()` | Gibt eine neue, sortierte Liste zurück, ohne die ursprüngliche Liste zu verändern. Dies ist besser und stabiler! | `sorted([3, 1, 2])` gibt `[1, 2, 3]` zurück<br>`sorted([3, 1, 2], reverse=True)` gibt `[3, 2, 1]` zurück |
