...
duty_cycle_high = 65535  # 100% Leistung
duty_cycle_low  = 13107  # 20% Leistung (65535 * 0.2)
state = False            # Startzustand der LED "aus"

def blink(timer):
    global state         # globale Variable wird übernommen
    if state:
        led.duty_u16(duty_cycle_low)  # Setze den Duty Cycle auf 20%
    else:
        led.duty_u16(duty_cycle_high)  # Setze den Duty Cycle auf 100%
...
!!! question "Sourcecode verstehen"
    Was ist im Sourcecode fest vorgegeben und was kann man *ändern wie man möchte*?
...
Der hier gezeigte Sourcecode basiert auf dem Sourcecode von [Elektronik Kompendium](https://www.elektronik-kompendium.de/sites/raspberry-pi/2703141.htm) und wurde nur leicht abgeändert.
...
# Original Sourcecode von Elektronik Kompendium 
...
| Befehl                   | Erklärung                                                   |
|--------------------------|-------------------------------------------------------------|
| from *bibliothek* import *Klasse*  | Importiert die Klasse aus der Bibliothek        |
| print(...)               | Gibt eine Nachricht auf der Konsole aus                     |
| def                      | Definiert eine Funktion                                     |
| if                       | Bedingte Anweisung                                          |
| elif                     | Weitere Bedingte Anweisung                                  |
| else                     | Alternative Bedingte Anweisung                              |
| while                    | Schleife, die solange eine Bedingung erfüllt ist, wiederholt wird |
