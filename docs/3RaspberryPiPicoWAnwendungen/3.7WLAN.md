...
??? example "2. MAC Adresse des eigene Pi Pico anzeigen"
    ...
    macAddress = ':'.join(['{:02x}'.format(b) for b in mac])
    print(f"Wlan-MAC-Adresse: {macAddress})
    ```
...
??? example "3. Ausführliches Script zur WLAN-Verbindung" 
    ...
    def connectToWlan(ssid, password):
        ...
        if not wlan.isconnected():
            ...
            for attempt in range(20):  # 20 Versuche, jeweils 0,5 Sekunden
                if wlan.isconnected():
                    ...
                    print(f"Verbindung wird aufgebaut, Versuch {attempt + 1}")
                    time.sleep(0.5)
    ...
    wlan = connectToWlan(SSID, PASSWORD)
...
# Initialisierung I2C-Pins und OLED-Display
i2c = SoftI2C(scl=Pin(17), sda=Pin(16))an
oled = SSD1306_I2C(128, 64, i2c, addr=0x3C)
...
??? info "Nerd-Wissen wlan.ifconfig()"
    Der Befehl `wlan.ifconfig()` liefert ein Tuple zurück (ein Tuple ist wie eine Liste, aber die Elemente können nicht verändert werden, analog zu den Begriffen Variable und Konstante). Das Tuple in `wlan.ifconfig()` besteht aus vier Elementen. Diese Elemente enthalten wichtige Netzwerkinformationen, wie die IP-Adresse und die Subnetzmaske, die das Gerät im Netzwerk verwendet. 

    Ein Tuple wird wie eine Liste über einen Index (hier also 0 bis 3 für die vier Elemente) angesprochen. Um auf einen spezifischen Wert aus dem Tuple, das von `wlan.ifconfig()`  zurückgegeben wird, zuzugreifen, verwendet man die eckigen Klammern `[]` und den entsprechenden Index. 

    Zum Beispiel:
    
    - `wlan.ifconfig()[0]` liefert die IP-Adresse.
    - `wlan.ifconfig()[1]` liefert die Subnetzmaske.
    - `wlan.ifconfig()[2]` liefert die Gateway-Adresse.
    - `wlan.ifconfig()[3]` liefert die DNS-Server-Adresse.
...
Der Pi Pico kann somit als Webclient verwendet werden. Im nächsten Abschnitt sehen wir, wie wir den Pi Pico als Webserver verwenden.